#include <string.h>
#include <stdio.h>
#include "sh1106.h"
#include "sh1106_tests.h"

//------------------------------------------------------------------------------
// Table generated by LCD Assistant
// http://en.radzio.dxp.pl/bitmap_converter/
//------------------------------------------------------------------------------
const unsigned char garfield_128x64 [] = {

};

const unsigned char github_logo_64x64[] = {

};


void sh1106_TestBorder() {
    sh1106_Fill(Black);
   
    uint8_t x = 0;
    uint8_t y = 0;
    do {
        sh1106_DrawPixel(x, y, Black);

        if((y == 0) && (x < (SH1106_WIDTH-1)))
            x++;
        else if((x == (SH1106_WIDTH-1)) && (y < (SH1106_HEIGHT-1)))
            y++;
        else if((y == (SH1106_HEIGHT-1)) && (x > 0))
            x--;
        else
            y--;

        sh1106_DrawPixel(x, y, White);
        sh1106_UpdateScreen();
    
        HAL_Delay(5);
    } while(x > 0 || y > 0);
   
    HAL_Delay(1000);
}

void sh1106_TestFonts1() {
    uint8_t y = 0;
    sh1106_Fill(Black);

    #ifdef SH1106_INCLUDE_FONT_16x26
    sh1106_SetCursor(2, y);
    sh1106_WriteString("Font 16x26", Font_16x26, White);
    y += 26;
    #endif

    #ifdef SH1106_INCLUDE_FONT_11x18
    sh1106_SetCursor(2, y);
    sh1106_WriteString("Font 11x18", Font_11x18, White);
    y += 18;
    #endif

    #ifdef SH1106_INCLUDE_FONT_7x10
    sh1106_SetCursor(2, y);
    sh1106_WriteString("Font 7x10", Font_7x10, White);
    y += 10;
    #endif

    #ifdef SH1106_INCLUDE_FONT_6x8
    sh1106_SetCursor(2, y);
    sh1106_WriteString("Font 6x8", Font_6x8, White);
    #endif

    sh1106_UpdateScreen();
}

/*
 * This test shows how an 128x64 px OLED can replace a 0802 LCD.
 */
void sh1106_TestFonts2() {
#ifdef SH1106_INCLUDE_FONT_16x24
    uint8_t x1, y1, x2, y2;

    sh1106_Fill(Black);

    sh1106_SetCursor(0, 4);
    sh1106_WriteString("18.092.5", Font_16x24, White);
    sh1106_SetCursor(0, 4+24+8);
    sh1106_WriteString("RIT+1000", Font_16x24, White);

    // underline
    x1 = 6*16;
    y1 = 4+24+8+24;
    x2 = x1+16;
    y2 = y1+2;
    sh1106_FillRectangle(x1, y1, x2, y2, White);

    sh1106_UpdateScreen();
#endif
}

void sh1106_TestFPS() {
    sh1106_Fill(White);
   
    uint32_t start = HAL_GetTick();
    uint32_t end = start;
    int fps = 0;
    char message[] = "ABCDEFGHIJK";
   
    sh1106_SetCursor(2,0);
    sh1106_WriteString("Testing...", Font_11x18, Black);
    sh1106_SetCursor(2, 18*2);
    sh1106_WriteString("0123456789A", Font_11x18, Black);
   
    do {
        sh1106_SetCursor(2, 18);
        sh1106_WriteString(message, Font_11x18, Black);
        sh1106_UpdateScreen();
       
        char ch = message[0];
        memmove(message, message+1, sizeof(message)-2);
        message[sizeof(message)-2] = ch;

        fps++;
        end = HAL_GetTick();
    } while((end - start) < 5000);
   
    HAL_Delay(5000);

    char buff[64];
    fps = (float)fps / ((end - start) / 1000.0);
    snprintf(buff, sizeof(buff), "~%d FPS", fps);
   
    sh1106_Fill(White);
    sh1106_SetCursor(2, 2);
    sh1106_WriteString(buff, Font_11x18, Black);
    sh1106_UpdateScreen();
}

void sh1106_TestLine() {

  sh1106_Line(1,1,SH1106_WIDTH - 1,SH1106_HEIGHT - 1,White);
  sh1106_Line(SH1106_WIDTH - 1,1,1,SH1106_HEIGHT - 1,White);
  sh1106_UpdateScreen();
  return;
}

void sh1106_TestRectangle() {
  uint32_t delta;

  for(delta = 0; delta < 5; delta ++) {
    sh1106_DrawRectangle(1 + (5*delta),1 + (5*delta) ,SH1106_WIDTH-1 - (5*delta),SH1106_HEIGHT-1 - (5*delta),White);
  }
  sh1106_UpdateScreen();
  return;
}

void sh1106_TestRectangleFill() {
  sh1106_FillRectangle(31, 1, 65, 35, White);
  sh1106_FillRectangle(10, 45, 70, 60, White);
  sh1106_FillRectangle(75, 10, 100, 45, White);
  sh1106_FillRectangle(55, 30, 80, 55, Black);
  sh1106_UpdateScreen();
}

void sh1106_TestCircle() {
  uint32_t delta;

  for(delta = 0; delta < 5; delta ++) {
    sh1106_DrawCircle(20*delta+30, 15, 10, White);
  }

  for(delta = 0; delta < 5; delta ++) {
    sh1106_FillCircle(23*delta+15, 40, 10, White);
  }

  sh1106_UpdateScreen();
  return;
}

void sh1106_TestArc() {
  sh1106_DrawArc(30, 30, 30, 20, 270, White);
  sh1106_DrawArcWithRadiusLine(80, 55, 10, 30, 270, White);
  sh1106_UpdateScreen();
  return;
}

void sh1106_TestPolyline() {
  SH1106_VERTEX loc_vertex[] =
  {
      {35,40},
      {40,20},
      {45,28},
      {50,10},
      {45,16},
      {50,10},
      {53,16}
  };

  sh1106_Polyline(loc_vertex,sizeof(loc_vertex)/sizeof(loc_vertex[0]),White);
  sh1106_UpdateScreen();
  return;
}

void sh1106_TestDrawBitmap()
{
    sh1106_Fill(White);
    sh1106_DrawBitmap(0,0,garfield_128x64,128,64,Black);
    sh1106_UpdateScreen();
    HAL_Delay(3000);
    sh1106_Fill(Black);
    sh1106_DrawBitmap(32,0,github_logo_64x64,64,64,White);
    sh1106_UpdateScreen();
    HAL_Delay(3000);
    sh1106_Fill(White);
    sh1106_DrawBitmap(32,0,github_logo_64x64,64,64,Black);
    sh1106_UpdateScreen();
    HAL_Delay(3000);
}

void sh1106_TestAll() {
    sh1106_Init();

    sh1106_TestFPS();
    HAL_Delay(3000);
    sh1106_TestBorder();
    sh1106_TestFonts1();
    HAL_Delay(3000);
    sh1106_TestFonts2();
    HAL_Delay(3000);
    sh1106_Fill(Black);
    sh1106_TestRectangle();
    sh1106_TestLine();
    HAL_Delay(3000);
    sh1106_Fill(Black);
    sh1106_TestRectangleFill();
    HAL_Delay(3000);
    sh1106_Fill(Black);
    sh1106_TestPolyline();
    HAL_Delay(3000);
    sh1106_Fill(Black);
    sh1106_TestArc();
    HAL_Delay(3000);
    sh1106_Fill(Black);
    sh1106_TestCircle();
    HAL_Delay(3000);
    sh1106_TestDrawBitmap();
    HAL_Delay(3000);
}

void sh1106_GraficarMPU6050(){

}
